...
contract('DittoClaimDistributor', async function (accounts) {
  let token
   ...
   * Initialize DittoClaimDistributor contract + seed it with tokens
   */
  beforeEach(async function () {
    // Deploy token
    token = await DITTO.new(tokenOwnerAddress, { from: tokenOwnerAddress })

    // Initialize merkle tree from accounts data set
    tree = new AccountAmountMerkleTree(accountClaimAmounts)
    merkleRoot = tree.getHexRoot()

    // Initialize DittoClaimDistributor contract
    distributor = await DittoClaimDistributor.new(token.address, merkleRoot, withdrawBlock, withdrawAddress)
  ...
  describe('Single claim tests', async function () {
    ...
    it('Confirm claim fails with invalid tree index', async function () {
      ...
      await _lib.assertRevert(
        ...
        'DittoClaimDistributor: Invalid proof.'
      )
    })
  ...
  describe('Multiple claims', async function () {
    it('Confirm repeated claims fail', async function () {
      ...
      await _lib.assertRevert(
        ...
        'DittoClaimDistributor: Drop already claimed.'
      )
    })
